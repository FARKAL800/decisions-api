...
from decision_api.security import admin_required
...
async def get_decisions(skip: int = 0, limit: int = 10):
    query = decisions_table.select().offset(skip).limit(limit)
...
async def get_decision(identifiant: str):
    query = decisions_table.select().where(decisions_table.c.identifiant == identifiant)
...
async def search_decisions(search: str):
    search_body = {
        "query": {
            "match": {
