...
from decision_api import security
...
def test_create_access_token():
    token = security.create_access_token("123", "user")
    assert {"sub": "123", "scope": "user"}.items() <= jwt.decode(
...
async def test_get_current_user(registered_user: dict):
    token = security.create_access_token(
        registered_user["email"], registered_user["scope"]
    )
